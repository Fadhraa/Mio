...
const rulesMiomi = ChatPromptTemplate.fromMessages([
  [
    "system",
    `Kamu adalah Miomi, asisten spesialis akademik yang teliti dan cerdas milik ${USER_NAME}.
Fokus utamamu HANYA membantu tugas sekolah/kuliah dan kegiatan belajar ${USER_NAME} menggunakan tool yang kamu miliki.
ATURAN PENTING:
- Jika ${USER_NAME} menyuruh membuat laporan, dokumen panjang, esai, atau google docs, gunakan tool 'kelola_google_doc'.
...
const miomiExecutor = new AgentExecutor({
  agent: agenMiomi,
  tools: toolsMiomi,
});

// 4. JADIKAN MIOMI SEBAGAI "TOOL" UNTUK MIO
export const panggilMiomi = tool(
  async ({ instruksi }) => {
    console.log(
  ...
  },
  {
    ...
    description:
      "PENTING: Gunakan alat ini JIKA pengguna meminta bantuan terkait akademik, tugas sekolah/kuliah, belajar, atau MENCATAT/MENYIMPAN tugas baru. Berikan instruksi yang jelas kepada Miomi agar dia bisa mengerjakannya.",
    schema: z.object({
      instruksi: z
        .string()
        .describe(
          "Perintah lengkap dan spesifik tentang apa yang harus Miomi kerjakan untuk pengguna.",
        ),
    }),
  },
);
